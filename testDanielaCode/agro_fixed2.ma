...
neighborports: amb mgm lu1 lu2 lu3 pro eme ua_tipo ua_cota ue_cota deg uae uao uee ueo alq etapa flag_paso flag_check_bool
...
rule: { 
		...
		~pro 	:=  (((0,0)~lu1/ 100) * ((#macro(rinde_lu1) * #macro(precio_lu1)) - #macro(costo_lu1))) + 
					...
					(((0,0)~lu3/ 100) * (((#macro(rinde_lu4) * #macro(precio_lu4)) 
					 + (#macro(rinde_lu5) * #macro(precio_lu2))) - #macro(costo_lu3))) - ((0,0)~alq  * (#macro(precio_lu2)));
...
rule: { 
	...
	{ 
		...
		(
			...
			(((-1,1)~pro = ?) or  (-1,1)~pro		<= (-1,0)~pro)	and
			(((0,1)~pro = ?) or  (0,1)~pro 		<= (-1,0)~pro)	and
			(((1,1)~pro = ?) or  (1,1)~pro 		<= (-1,0)~pro)	and
			(((1,0)~pro = ?) or  (1,0)~pro 		<= (-1,0)~pro)	and
			(((1,-1)~pro = ?) or  (1,-1)~pro 	<= (-1,0)~pro)	and
			(((0,-1)~pro = ?) or  (0,-1)~pro 	<= (-1,0)~pro)	and
			(((-1,-1)~pro = ?) or  (-1,-1)~pro 	<= (-1,0)~pro)		
		)
	}
...
rule: { 
	...
	{ 
		...
		(
			...
			(((0,1)~pro = ?) or  (0,1)~pro 		<= (-1,1)~pro)	and
			(((1,1)~pro = ?) or  (1,1)~pro 		<= (-1,1)~pro)	and
			(((1,0)~pro = ?) or  (1,0)~pro 		<= (-1,1)~pro)	and
			(((1,-1)~pro = ?) or  (1,-1)~pro 	<= (-1,1)~pro)	and
			(((0,-1)~pro = ?) or  (0,-1)~pro 	<= (-1,1)~pro)	and
			(((-1,-1)~pro = ?) or  (-1,-1)~pro 	<= (-1,1)~pro)	and	
			(((-1,0)~pro = ?) or  (-1,0)~pro 	<= (-1,1)~pro)
		)									
	}
...
rule: { 
	...
	{ 
		...
		(
			...
			(((1,1)~pro = ?) or  (1,1)~pro 		<= (0,1)~pro)	and
			(((1,0)~pro = ?) or  (1,0)~pro 		<= (0,1)~pro)	and
			(((1,-1)~pro = ?) or  (1,-1)~pro		<= (0,1)~pro)	and
			(((0,-1)~pro = ?) or  (0,-1)~pro		<= (0,1)~pro)	and
			(((-1,-1)~pro = ?) or  (-1,-1)~pro 	<= (0,1)~pro)	and	
			(((-1,0)~pro = ?) or  (-1,0)~pro		<= (0,1)~pro)	and
			(((-1,1)~pro = ?) or  (-1,1)~pro		<= (0,1)~pro)
		)
	}
...
rule: { 
		...
		~flag_paso := 5.1 ;
		%~flag_check_bool :=  if (#macro(vecinosUeControlado),1,0);
		% skipeo el paso 5 (a ver que pasa)
...
rule: { 
	...
	{ 
		...
		#macro(vecinosUeControlado) 	%and  (2 = 1)
	}	
...
% Rule de testeo.
	rule: { 
		#macro(SetEtapaProcesamiento)
  		~flag_check_bool := if ((0,0)#macro(UECumplida),1,0);
		~flag_paso := 6.9 ;
	}
	{ }
	 0
	{ 
		(0,0)~etapa = 5 %#macro(UECumplida)
	}
...
rule: { 
	...
	{ 
		...
		#macro(vecinosProcesamiento) 	%and (2 = 1) agregado
	}	
